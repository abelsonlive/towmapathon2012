(function(){
    ...
    var userName = 'tow-muroidea'
      , polygonTable = 'tracts'
      , polygonTableActiveCols = 'id2, median_rent'
      , pointTable = 'rats'
      , pointTableActiveCols = 'rattiness'
      , defaultZoom = 13
      , markerWidth = 5
      ...
      , $adrsGo = $('#pfx_inputField')
        // make your map
      , map = new L.Map(mapCanvas).setView(new L.LatLng(40.77, -73.969), defaultZoom)
    ...
    //Begin format helper functions
    var formatHelpers = {
        addCommas: function(numb){
              return numb.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")
        }, //make sure that commas is there if you're going to add another function to formatHelpers
        makeCurrency: function(numb){
              return '$' +  numb.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")
        }
    }
    ...
    $('.pfx_radioBox input').click(function(){
      ...
      if (displayVar != 'all'){
        ptTable.setQuery('SELECT * FROM' + pointTable);
        polygonTable.setQuery('');
      }
  });
      // BEGIN GEOCODING
  var geocoder
  ...
  var geoCode = function(address){
    geocoder.geocode( { 'address': address}, function(results, status) {
        if (status == google.maps.GeocoderStatus.OK) {
          ...
          if (firstRun == false){
            mapCanvas.removeLayer(markerGroup)
          }
